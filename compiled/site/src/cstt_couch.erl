...
create_db(DbName) ->
    create_db(DbName, [?ADMIN_USER_CTX]).
...
open_attachment(Db, DocId, FileName, Options) ->
    case open_attachment_async(Db, DocId, FileName, Options) of
        {ok, Pid, ranges, Type} ->
            {ok, collect_attachment_ranges(Pid), Type};
            
        {ok, Pid, {content_md5, _}=_Md5, Type} ->
            %{ok, collect_attachment(Pid), Md5};
            {ok, collect_attachment(Pid), Type};
        {ok, Pid, Type} ->
...
%% ranges}
open_attachment_async(DbName, DocId, FileName, Options) ->
	{ok, Db} = cstt_couch:open_db(DbName),
                                ...
                                case {Enc, Ranges} of
                                    ...
                                    _ ->
                                        ...
                                        if Enc =:= identity orelse AcceptsAttEnc =:= true ->
                                            {ok, Pid, {content_md5,
                                                    base64:encode(Att#att.md5)}, Type};
                                        true ->
                                            {ok, Pid, Type}
                                        end
                                end
...
query_view(DbName, DesignName, VName) ->
	
	{ok, Db} = couch_db:open(dbname(DbName), [?ADMIN_USER_CTX]),
	DesignId = <<"_design/", DesignName/binary>>,
	
    DDoc = couch_httpd_db:couch_doc_open(Db, DesignId, nil, [ejson_body]),
    
    {ok, Resp} = couch_mrview:query_view(Db, DDoc, VName, #mrargs{}),
...
query_view(DbName, DesignName, VName, Args) ->
	{ok, Db} = couch_db:open(dbname(DbName), [?ADMIN_USER_CTX]),
	
	DesignId = <<"_design/", DesignName/binary>>,
	
    DDoc = couch_httpd_db:couch_doc_open(Db, DesignId, nil, [ejson_body]),
	couch_mrview:query_view(Db, DDoc, VName, Args).
