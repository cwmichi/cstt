...
function renderer_product (val, x, store) {
	if(!Ext.isEmpty(val)) {
		var index, text;
		index = Ext.data.StoreManager.lookup('productstore').findExact('value', val);
		if (index >= 0) {
		text = Ext.data.StoreManager.lookup('productstore').getAt(index).data.name;
		return text
		} else {
			return "Cordaware bestinformed";	
		}
	} else {
		return "Cordaware bestinformed";
	}
